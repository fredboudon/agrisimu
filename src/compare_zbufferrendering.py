...
class PglViewer (QGLViewer):
    def __init__(self, parent = None, label = None):
        ...
        self.camera().setPosition(Vec(27.9,5.4,2))
        self.camera().setViewDirection(Vec(0,0,1))
...
def main():
    from generateplot import agristructure
    qapp = QApplication([])
    ...
    widget.show()
    #right.show()
    right.camera().setPosition(Vec(27.9,5.4,2))
    right.camera().setViewDirection(Vec(0,0,1))
    right.camera().setUpVector(Vec(0,-1,0))
